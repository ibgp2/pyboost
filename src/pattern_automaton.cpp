    ...
    word(word)
{
    ...
    std::cout << this->adjacencies << std::endl;
}
...
void PatternAutomaton::add_edge(State q, State r, Label a) {
    ...
    if (std::size_t(q) < n && std::size_t(r) < n && a < this->get_alphabet_size()) {
        ...
        message << "add_edge(q = " << q
            << ", r = " << r
            << ", a = " + a << "):"
            << " q and r must be lower than num_vertices = " << n
            << " a must be lower than alphabet_size = " << this->get_alphabet_size()
            ;
        throw std::runtime_error(message.str());
    }
}

int PatternAutomaton::delta(State q, Label a) const {
    return this->adjacencies[q][a];
}
...
const std::string & PatternAutomaton::get_word() const {
    return this->word;
}
